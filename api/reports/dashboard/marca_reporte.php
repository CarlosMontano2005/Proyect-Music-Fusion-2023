...
require_once('../../entities/Controller/Controller_Marcas_dto.php');
...
if ($dataMarca = $marca->readAll()) {
    ...
    $pdf->cell(122, 10, 'Nombre', 1, 0, 'C', 1); //(Ancho, Alto, Titulo, borde 1 - 0, salto de linea L-C-R, color de fondo 1-0)
    $pdf->cell(25, 10, 'Imagen', 1, 1, 'C', 1);
    ...
    foreach ($dataMarca as $rowMarca) {
        ...
        $pdf->cell(20, 20, $rowMarca['id_marca_producto'], 1, 0,'C');
        $pdf->cell(122, 20, $rowMarca['nombre_marca'], 1, 0,'C');
        //${SERVER_URL}img/people/${row.foto}
        // $this->cell(0, 10, $this->encodeString('Página ').$this->pageNo().'/{nb}', 0, 0, 'C');
        // $pdf->Image($pdf->encodeString('../../img/people/'.$rowUsuario['foto']), 30.375, 10, -300);
        $pdf->cell(25, 1,$pdf->Image($pdf->encodeString('../../img/marcas/'.$rowMarca['logo_marca']),null,null,20.5,20), 1, 1, 'C');//indicar el 1,1 al final para que sea una sola fila cada dato 
